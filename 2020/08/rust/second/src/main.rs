...
fn main() {
    ...
    if let Ok(lines) = read_lines(filename) {
        ...
        for changed_idx in 0..instructions.len() {
            let mut copied_instructions = instructions.clone();
            match instructions[changed_idx].0.as_str() {
                "nop" => copied_instructions[changed_idx].0 = "jmp".to_string(),
                "jmp" => copied_instructions[changed_idx].0 = "nop".to_string(),
                _ => println!("Nop")
            }
            ...
            loop {
                if visited.contains(&idx) {
                    ...
                    break;
                } else if idx >= copied_instructions.len() {
                    println!("Perfect !");
                    println!("Acc: {}", acc);
                    return;
                } else {
                ...
                let (instruction, argument) = &copied_instructions[idx];
